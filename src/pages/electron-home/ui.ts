...
export const ICON_BUTTON_CLASS = `inline-flex h-8 w-8 items-center justify-center rounded-md text-text-subtle transition-[background-color,border-color,color,transform] duration-150 ease-out hover:bg-element-bg-hover hover:text-text-default motion-reduce:transition-none ${PRESSED_CLASS} ${FOCUS_RING_CLASS} disabled:pointer-events-none disabled:opacity-50`;
export const SECONDARY_BUTTON_CLASS = `inline-flex h-9 items-center gap-2 rounded-md border border-border-default px-3 text-sm transition-colors hover:bg-element-bg-hover active:bg-background-selected ${PRESSED_CLASS} ${FOCUS_RING_CLASS}`;
